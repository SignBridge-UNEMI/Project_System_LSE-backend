...
def read_frames_from_directory(directory):
    """
    Lee todos los frames (imágenes) de un directorio específico.

    Args:
        directory (str): La ruta al directorio que contiene las imágenes.

    Returns:
        list: Una lista de frames (imágenes) leídas desde el directorio.
    """
    frames = []
...
def interpolate_frames(frames, target_frame_count=15):
    """
    Interpola los frames existentes para alcanzar un número objetivo de frames.

    Args:
        frames (list): Lista de frames (imágenes) a interpolar.
        target_frame_count (int): El número objetivo de frames.

    Returns:
        list: Lista de frames interpolados.
    """
...
def normalize_frames(frames, target_frame_count=15):
    """
    Normaliza el número de frames a un número objetivo, ya sea interpolando o
    seleccionando frames existentes.

    Args:
        frames (list): Lista de frames (imágenes) a normalizar.
        target_frame_count (int): El número objetivo de frames.

    Returns:
        list: Lista de frames normalizados.
    """
    current_frame_count = len(frames)
    if current_frame_count < target_frame_count:
...
def process_directory(word_directory, target_frame_count=15):
    """
    Procesa todos los subdirectorios dentro de un directorio específico, normalizando
    los frames en cada subdirectorio.

    Args:
        word_directory (str): Ruta al directorio que contiene subdirectorios de palabras.
        target_frame_count (int): El número objetivo de frames para normalizar.
    """
...
def save_normalized_frames(directory, frames):
    """
    Guarda los frames normalizados en un directorio específico.

    Args:
        directory (str): Ruta al directorio donde se guardarán las imágenes.
        frames (list): Lista de frames (imágenes) normalizadas a guardar.
    """
...
def clear_directory(directory):
    """
    Elimina todos los archivos y subdirectorios en un directorio específico.

    Args:
        directory (str): Ruta al directorio a limpiar.
    """
...
if __name__ == "__main__":
    ...
    # Uncomment to test normalization for a specific sample
