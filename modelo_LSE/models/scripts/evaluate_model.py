...
def interpolate_keypoints(keypoints, target_length=15):
    """Interpolate keypoints to match the target length."""
    ...
    for i in indices:
        ...
        if lower_idx == upper_idx:
            ...
            interpolated_point = (
                (1 - weight) * np.array(keypoints[lower_idx]) +
                weight * np.array(keypoints[upper_idx])
            )
...
def normalize_keypoints(keypoints, target_length=15):
    """Normalize the keypoints to the target length by interpolating or downsampling."""
    current_length = len(keypoints)
    if current_length < target_length:
...
def evaluate_model(src=None, threshold=0.8, margin_frame=1, delay_frames=3):
    """Evaluate the model for sign language recognition from a video source."""
    kp_seq, sentence = [], []
    ...
    with Holistic() as holistic_model:
        ...
        while video.isOpened():
            ...
            # Detect hand presence and record keypoints
            if there_hand(results) or recording:
                ...
                if count_frame >= MIN_LENGTH_FRAMES + margin_frame:
                    ...
                    # Normalize keypoints for prediction
                    ...
                    print(predicted_index, f"({predicted_confidence * 100:.2f}%)")
